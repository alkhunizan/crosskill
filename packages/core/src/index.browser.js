...
var TargetsSchema = exports_external.object({
  claude: exports_external.boolean().optional(),
  cursor: exports_external.boolean().optional(),
  codex: exports_external.boolean().optional(),
  windsurf: exports_external.boolean().optional(),
  aider: exports_external.boolean().optional(),
  opencode: exports_external.boolean().optional(),
  gemini: exports_external.boolean().optional(),
  copilot: exports_external.boolean().optional(),
  continue: exports_external.boolean().optional()
}).default({});
var SkillExampleSchema = exports_external.object({
  input: exports_external.string().min(1),
  contains: exports_external.array(exports_external.string()).optional(),
  matches: exports_external.string().optional()
});
var SkillFrontmatterSchema = exports_external.object({
  name: exports_external.string().min(1).regex(/^[a-z0-9][a-z0-9-]*[a-z0-9]$/, "name must be kebab-case (lowercase, digits, hyphens)"),
  version: exports_external.string().regex(/^\d+\.\d+\.\d+(-[a-z0-9.-]+)?$/i, "version must be valid semver, e.g. 0.1.0").default("0.1.0"),
  description: exports_external.string().min(8).max(240),
  author: exports_external.string().optional(),
  tags: exports_external.array(exports_external.string()).default([]),
  license: exports_external.string().default("MIT"),
  targets: TargetsSchema,
  tools: exports_external.array(exports_external.string()).optional(),
  inputs: exports_external.array(exports_external.string()).optional(),
  examples: exports_external.array(SkillExampleSchema).optional(),
  homepage: exports_external.string().url().optional()
});

// src/parser.ts
function parseSkillString(raw, sourcePath) {
...
`);
    throw new SkillParseError(`Invalid front-matter in ${fileName}
${issues}`, where);
  }
  const body = parsed.content.trim();
  if (!body) {
    throw new SkillParseError(`Skill body is empty in ${sourcePath ?? "<inline>"}. The body is the prompt itself.`, sourcePath ?? "<inline>");
  }
  return {
...
function lintSkill(skill, config = DEFAULT_LINT_CONFIG) {
  const issues = [];
...
// src/compilers/windsurf.ts
var windsurfCompiler = {
  target: "windsurf",
...
${skill.body}
`;
    const outputPath = joinPath(outputRoot, `.windsurf/rules/${name}.md`);
    return { target: "windsurf", outputPath, content };
  }
};

// src/compilers/aider.ts
var aiderCompiler = {
  target: "aider",
...
// src/compilers/opencode.ts
var opencodeCompiler = {
  target: "opencode",
...
// src/compilers/continue.ts
var continueCompiler = {
  target: "continue",
...
// src/compile.ts
function compileSkill(skill, target, outputRoot = ".") {
  const parsed = typeof skill === "string" ? parseSkillString(skill) : skill;
  const compiler = COMPILERS[target];
  if (!compiler) {
    throw new Error(`No compiler registered for target "${target}". Available: ${Object.entries(COMPILERS).filter(([, c]) => c).map(([k]) => k).join(", ")}`);
  }
  return compiler.compile(parsed, outputRoot);
}
function compileSkillAll(skill, outputRoot = ".") {
  const parsed = typeof skill === "string" ? parseSkillString(skill) : skill;
...
function compileSkillForPreview(source, opts = {}) {
  let skill;
  try {
  ...
  const wantedTargets = opts.onlyTargets ? new Set(opts.onlyTargets) : null;
  const outputRoot = opts.outputRoot ?? ".";
  const compiled = [];
  for (const target of Object.keys(enabled)) {
    if (!enabled[target])
      continue;
    if (wantedTargets && !wantedTargets.has(target))
      continue;
    const compiler = COMPILERS[target];
    if (!compiler)
      continue;
...
//# debugId=E97F995CB6BD47FF64756E2164756E21
